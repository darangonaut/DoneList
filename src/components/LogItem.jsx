...
export const LogItem = memo(function LogItem({ log, onDelete, onUpdate, onTagClick, onShare, isSelectable = false, onSelect, getTagColor }) {
      ...
      if (part.startsWith('#')) {
        ...
        return (
          <span 
            ...
            className="inline-block px-3 py-1 rounded-md text-[14px] font-bold mx-0.5 cursor-pointer active:scale-90 transition-transform relative z-20 shadow-sm border border-black/10" 
  ...
  return (
    <motion.div 
      ...
      {!isSelectable && (
        <motion.div style={{ opacity, scale }} className="absolute inset-y-0 right-0 w-20 bg-red-600 flex items-center justify-center z-0">
          <button 
      ...
      <motion.div 
        ...
        className={`bg-apple-card/90 backdrop-blur-2xl p-5 border flex justify-between items-center relative z-10 rounded-[2rem] touch-pan-y
      ...
      >
        <div className="flex flex-col pr-4 flex-1 select-none text-apple-text">
          {isEditing ? (
            ...
            <div 
              ...
              className="text-[17px] leading-tight font-bold cursor-text whitespace-pre-wrap break-words flex items-start gap-2 select-none touch-manipulation"
            >
          ...
          <span className="text-[13px] font-black text-apple-secondary mt-1.5">{formatTimestamp(log.timestamp)}</span>
        </div>
        ...
        {isSpecial && !isSelectable && !isEditing && (
          <button 
            ...
            className="p-4 -mr-4 text-apple-text active:scale-90 transition-transform opacity-100"
          >
