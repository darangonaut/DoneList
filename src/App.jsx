      ...
      <div className="relative z-10 w-full max-w-2xl mx-auto px-6 py-20">
        <section className="min-h-[80vh] flex flex-col items-center justify-center text-center mb-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="w-full flex flex-col items-center">
            ...
            <button onClick={handleLogin} className="w-full max-w-sm bg-white text-black py-6 rounded-[2rem] font-black text-xl shadow-[0_25px_50px_rgba(255,255,255,0.15)] active:scale-95 transition-all flex items-center justify-center gap-4 mx-auto">
        ...
        <section className="space-y-24 py-20 border-t border-white/10">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center max-w-lg mx-auto">
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-8">{t.promoTitle}</h2>
            <p className="text-lg text-white/50 leading-relaxed italic">"{t.promoIntro}"</p>
          </motion.div>

          <div className="grid gap-8">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white/5 border border-white/10 p-8 rounded-[2.5rem] flex flex-col md:flex-row gap-6 items-start">
              <span className="text-5xl shrink-0">🧠</span>
              <div>
                <h3 className="text-2xl font-bold mb-3">{t.promo1Title}</h3>
                <p className="text-white/50 text-lg leading-relaxed">{t.promo1Desc}</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white/5 border border-white/10 p-8 rounded-[2.5rem] flex flex-col md:flex-row gap-6 items-start">
              ...
              <div>
                <h3 className="text-2xl font-bold mb-3">{t.promo2Title}</h3>
                <p className="text-white/50 text-lg leading-relaxed">{t.promo2Desc}</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white/5 border border-white/10 p-8 rounded-[2.5rem] flex flex-col md:flex-row gap-6 items-start">
              <span className="text-5xl shrink-0">📈</span>
              <div>
                <h3 className="text-2xl font-bold mb-3">{t.promo3Title}</h3>
                <p className="text-white/50 text-lg leading-relaxed">{t.promo3Desc}</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white/5 border border-white/10 p-8 rounded-[2.5rem] flex flex-col md:flex-row gap-6 items-start">
              <span className="text-5xl shrink-0">⛽️</span>
              <div>
                <h3 className="text-2xl font-bold mb-3">{t.promo4Title}</h3>
                <p className="text-white/50 text-lg leading-relaxed">{t.promo4Desc}</p>
              </div>
            </motion.div>
          </div>
        </section>
        ...
        <section className="py-20 border-t border-white/10 w-full text-center">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-[var(--accent-color)]/10 border border-[var(--accent-color)]/20 p-8 rounded-[2.5rem] max-w-lg mx-auto">
            <h3 className="text-xl font-bold mb-3 text-[var(--accent-color)]">{t.tipTitle}</h3>
            <p className="text-white/70 text-lg leading-relaxed">{t.tipDesc}</p>
          </motion.div>
        </section>

        <section className="py-20 text-center">
          <button onClick={handleLogin} className="w-full max-w-sm bg-white text-black py-6 rounded-[2rem] font-black text-xl shadow-[0_25px_50px_rgba(255,255,255,0.15)] active:scale-95 transition-all mb-12 mx-auto block">
            {t.login}
          </button>
          ...
          <AnimatePresence initial={false}>
            {logs.map((log) => (
              <motion.div key={log.id} layout initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0, transition: { duration: 0.2 } }}>
                <LogItem log={log} onDelete={() => { if (!loading) deleteDoc(doc(db, 'logs', log.id)); }} />
              </motion.div>
            ))}
          </AnimatePresence>
